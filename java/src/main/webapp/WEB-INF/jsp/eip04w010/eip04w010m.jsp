...
<!doctype html>
<spring:eval var="caseKey" expression="T(tw.gov.pcc.eip.common.controllers.Eip04w010Controller).CASE_KEY" />
...
<jsp:attribute name="contents">
    ...
    <form:form id="eip04w010Form" name="eip04w010Form" modelAttribute="${caseKey}" method="POST">
        <tags:form-row>
                ...
                <table class="table" id="tb1">
                    ...
                    <tbody>
                        <c:forEach items="${caseData.orcList}" var="item" varStatus="status">
                            <tr>
                                <td class="text-center align-middle"><form:checkbox path="orccodeList" data-starting="${item.starting}" value="${item.orccode}"/></td>
...
$(function(){
	...
	$('#btnInsert').click(function(e){
        e.preventDefault();
        $('#mode').val('A');
		$('#eip04w010Form').attr('action', '<c:url value="/Eip04w010_selectAction.action" />').submit();
	})
    $('#btnDelete').click(function(e){
        ...
        showConfirm('確定刪除嗎？',()=>{
            $('#eip04w010Form').attr('action', '<c:url value="/Eip04w010_delete.action" />').submit();
        });
    })
