...
<spring:eval var="manualKey" expression="T(tw.gov.pcc.eip.common.controllers.Eip00w420Controller).MANUAL_CASE_KEY" />
...
<form:form id="eip00w420Form" modelAttribute="${manualKey}" enctype="multipart/form-data" method="post">
    <tags:form-row>
         <div class="col-md-6">
    ...
    <tags:form-row>
        <div class="col-md-6">
            ...
            <form:input path="regisname" cssClass="form-control d-inline-block" size="20" maxlength="33"/>
        </div>
    </tags:form-row>
    <tags:form-row>
        <div class="col-md-6">
    ...
    <tags:form-row>
        <div class="col-md-6">
            ...
            <form:input path="regisphone" cssClass="form-control d-inline-block tel" size="12" maxlength="20"/>
        </div>
    </tags:form-row>
    <tags:form-row>
        <div class="col-md-6">
            <form:label cssClass="col-form-label star" path="jogtitle">職稱：</form:label>
            <form:input path="jogtitle" cssClass="form-control d-inline-block" size="20" maxlength="33"/>
        </div>
    </tags:form-row>
    <tags:form-row>
        <div class="col-md-6">
    ...
    <tags:form-row>
        <div class="col-md-10">
            ...
            <form:input path="dept" cssClass="form-control d-inline-block" size="50" maxlength="33"/>
        </div>
    </tags:form-row>
    <tags:form-row>
        <div class="col-md-10">
...
<script>
    $(function(){
        $('#btnSave').click(function(e){
            ...
            if(regType==='S') {
                $('#eip00w420Form').attr('action', '<c:url value="/Eip00w420_manualRegSingle.action" />').submit();
            } else if (regType==='B'){
                if ($.trim($('#file').val()).length === 0) {
                    showAlert("「檔案上傳」為必填");
                } else {
                    $('#eip00w420Form').attr('action', '<c:url value="/Eip00w420_manualRegBatch.action" />').submit();
                }
            }
        })
        $('#btnDownload').click(function(e){
            e.preventDefault();
            $('#eip00w420Form').attr('action', '<c:url value="/Eip00w420_downloadExample.action" />').submit();
        })
