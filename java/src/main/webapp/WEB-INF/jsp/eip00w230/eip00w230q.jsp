...
<!doctype html>
<spring:eval var="caseKey" expression="T(tw.gov.pcc.eip.adm.controllers.Eip00w230Controller).CASE_KEY"/>
...
<jsp:attribute name="heads">
    ...
    <form:form id="eip00w230Form" name="eip00w230Form" modelAttribute="${caseKey}" method="POST">
...
<script>
    ...
    $(function () {
        ...
        $('#btnBack').click(function () {
            $('#eip00w230Form').attr('action', '<c:url value="/Eip00w230_enter.action" />').submit();
        })
        $('#btnEdit').click(function () {
            $('#pickTabListString').val(sortable.toArray().join(','));
            $('#eip00w230Form').attr('action', '<c:url value="/Eip00w230_save.action" />').submit();
        })
    })
</script>
</jsp:attribute>
