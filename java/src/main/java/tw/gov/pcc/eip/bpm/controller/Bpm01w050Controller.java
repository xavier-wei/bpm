...
@Controller
@Slf4j
public class Bpm01w050Controller {

    public static final String CASE_KEY = "_bpm01w050Controller_caseData";
    private static final String MAIN_PAGE = "/bpm/Bpm01w050";//主頁
    private final String MAPPING_PATH = "/Bpm01w050_enter.action";
    private final UserBean userData;
    private final AESEncryptionService aesEncryptionService;

    public Bpm01w050Controller(UserBean userData, AESEncryptionService aesEncryptionService) {
        this.userData = userData;
        this.aesEncryptionService = aesEncryptionService;
    }

    @RequestMapping(MAPPING_PATH)
    public ModelAndView setSupervisor(HttpServletRequest request) {
        ...
        if (!isBpmLogin) {
            ...
            StringBuilder path = new StringBuilder(referer)
                    ...
                    .append("?referer=")
                    .append(referer)
                    .append("&path=/Bpm01w050_enter.action&token=")
                    .append(token);
        ...
        log.info("BPM表單管理:: 導向{}頁面","Bpm01w050 BPM表單簽核上級管理");
        return new ModelAndView(MAIN_PAGE).addAllObjects(Map.of("bpmPath", referer + "/bpm/supervisorAdmin"));
    }
}
