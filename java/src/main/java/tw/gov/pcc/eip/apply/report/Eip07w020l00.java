...
public class Eip07w020l00 extends PdfReportBase {
	 ...
	 * @throws Exception
	 */
	public void createEip07w020DataPdf(CarBooking caseData) throws Exception {
		try {
			document.open();
	 ...
	 * 列印單頁:單一印表頁次
	 */
	private void printSinglePage(CarBooking caseData) throws DocumentException {
		addHeader(caseData);
		addApplyData(caseData);
		if ("Y".equals(caseData.getCombine_mk())){
			combineMk(caseData);
		}
		addCarData(caseData);
	}
