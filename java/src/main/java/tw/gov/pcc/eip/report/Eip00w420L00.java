...
public class Eip00w420L00 {
	...
	public Eip00w420L00(String signform, String topicname, String time, String address, List<Orresult>list) {
	...
	public ByteArrayOutputStream creatWord(Eip00w420Case eip00w420Case) throws IOException {
...
//		FileOutputStream out = new FileOutputStream(new File("D:/wordDev/Word.docx"));
	...
	private void addDataRow(XWPFTable xwpfTable, Orresult orresult) {
		...
		for(int i=0;i<4;i++){
			...
			cell.removeParagraph(0);
			if(i == 0) {
				run.setText(orresult.getDept());
			}
			if(i == 1) {
				run.setText(orresult.getJogtitle());
			}
			if(i == 2) {
