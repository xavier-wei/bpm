...
export default function (idn: string): boolean {
  if (!helpers.req(idn)) {
    return true;
  }

  idn = idn.replace(/\s+/g, '');
  if (!/^[a-zA-Z]{1}[1-2]{1}[0-9]{8}$/.test(idn)) {
    return false;
  }
  ...
  const digit = locationMap[idn.toUpperCase().charCodeAt(0) - 'A'.charCodeAt(0)];
  let checksum = 0;
  ...
  for (let i = 1, p = 8; i <= 8; i++, p--) {
    checksum += parseInt(idn.charAt(i)) * p;
  }
  // add the last number
  checksum += parseInt(idn.charAt(9));
  return !(checksum % 10);
}
