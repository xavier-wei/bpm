  ...
  <b-card no-body :class="{ 'border-0': border0 }" v-if="$attrs.items !== undefined || state.itemsUndefinedBehaviorComputed">
    ...
    >
      ...
      <div style="margin-left: -20px" v-else>
        <b-form-group
          class="col-4 pl-0 mb-0 float-left tableSearch"
          label="篩選："
    ...
    <b-card-body class="m-0 p-0">
      <div id="buttonGroup" v-if="addButton || downloadButton || statisticsButton || idkWhatIsThisButton || exportCSVButton">
    ...
    >
      ...
      <b-row v-if="page === 'mrp0101r' || page === 'mrp0201r'">
        <div id="buttonGroup">
          <button class="mt-1 ml-1 btn btn-info" v-if="page === 'mrp0101r'" @click="download('')">下載</button>
          <button class="mt-1 ml-1 btn btn-info" v-if="page === 'mrp0201r'" @click="download('')">下載</button>
          <button class="mt-1 ml-1 btn btn-info" v-if="footerQueryButton">查詢集合</button>
        </div>
      </b-row>
    </div>
  </b-card>
