...
public class AuthorizeFilter implements Filter {
    ...
    private static final String SSO_AUTO_RE_LOGIN_URL = "ssoAutoReLoginUrl";
    ...
    private String ssoAutoReLoginUrl = null;
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        if (isLoginUrl(path)) {
            ...
            if (StringUtils.indexOf(req.getRequestURI(), unauthorizedUrl) == -1 && StringUtils.indexOf(req.getRequestURI(), undefineFunctionUrl) == -1 && StringUtils.indexOf(req.getRequestURI(), redirectUrl) == -1) {
                ...
                if (userData == null && StringUtils.isNotBlank(SpringHelper.getBeanByClass(LoginService.class).getUserIdFromFido())) {
                ...
                if (bSuccess) {
                    ...
                    // 檢核失敗
                    log.info("檢核失敗, 使用者代碼: {} 路徑: {}", StrUtil.safeLog(userData.getUserId()), StrUtil.safeLog(path));
                    if (!bFunctionDefined) {
