...
public class Eip08w030Service {
	...
	public void getCaseData(Eip08w030Case caseData) {
		List<Applyitem>list = applyitemDao.selectByApply_dateAndProcess_status(caseData.getApply_dateStart(), caseData.getApply_dateEnd());
	...
	public void updateAll(Eip08w030Case caseData) throws Exception{
		List<String> applynos = caseData.getDataList().stream().filter(it ->it.isCheck())
				.filter(it->StringUtils.isNotEmpty(it.getApplyno())).map(e -> e.getApplyno()).collect(Collectors.toList());
...
//		 P_book_cnt= STEP2查詢結果book_cnt + p_withhold_cnt
		...
		for(Applyitem item : dataN) {
			itemcodeService.applyUpdateItemcodeAndInsertItemcodeu(item.getItemkind(), item.getItemno(), item.getWithhold_cnt_a(), nowdatetime, "2");
			Itemcode itc = new Itemcode();
			itc.setItemkind(item.getItemkind());
			itc.setItemno(item.getItemno());
			Itemcode newData = itemcodeDao.selectDataByPrimaryKey(itc);
			newData.setWithhold_cnt(newData.getWithhold_cnt()-item.getWithhold_cnt_a());
			newData.setBook_cnt(newData.getBook_cnt()+item.getWithhold_cnt_a());
			newData.setUpd_user(userData.getUserId());
			newData.setUpd_datetime(nowdatetime);
			itemcodeDao.updateByKey(newData);
		}
